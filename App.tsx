...
const BackgroundMusic = ({ playing }: { playing: boolean }) => {
    useEffect(() => {
        ...
        return () => { if (osc) { try { osc.stop(); } catch (e) { } } if (ctx && ctx.state !== 'closed') { ctx.close(); } };
    }, [playing]);
    return null;
};
...
const HostApp = ({ theme, lang, onBack }: { theme: Theme, lang: Language, onBack: () => void }) => {
  ...
  if (gameState === GameState.LOBBY && session) {
      ...
      return (
      <Layout theme={theme}>
        <NewsTicker headlines={newsHeadlines} lang={lang} /> <MusicToggle playing={musicOn} onToggle={() => setMusicOn(!musicOn)} /> <LocalhostWarning />
